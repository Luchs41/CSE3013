...
void createRankList(){
	...
	else
	{
		...
		for(i=0;i<ranknum;i++)
		{
			fscanf(fp,"%s", myname);
			fscanf(fp,"%d",&myscore);
			firstInsertNode(myscore,myname,&head);
		}
	}
}
...
void newRank(int score){
	...
	noecho();
	ranknum++;
	insertNewNode(score,yourname,&head);
...
int recommend(RecNode *root){
	...
	for (r = 0;r<NUM_OF_ROTATE;++r)
	{
		...
		for (x = lBoundary;x <= rBoundary; x++, ++i)
		{
			...
			for (a = 0; a<HEIGHT; ++a)
				for (b = 0; b<WIDTH; ++b)
					c[i]->field[a][b] = root->field[a][b];
			c[i]->lv=lv;
			y = 0;
			
			if(CheckToMove(c[i]->field,nextBlock[lv],r,y,x)){
				while (CheckToMove(c[i]->field, nextBlock[lv], r,y+1, x))
					y++;
			}
			
			c[i]->accumulatedscore = root->accumulatedscore + checkScore(lv, c[i]->field, r, y, x);
			...
			if (max< eval&&CheckToMove(field,nextBlock[lv],r,y,x))
			{
				recommended = 1;
				solX = x;
				solR = r;
...
void firstInsertNode(int score, char name[NAMELEN], node** head)
{
...
void searchRank(char inputname[NAMELEN],node* head)
{
	int enter=0;
	int i;
	node* travel;
...
void constructRecTree(RecNode* root)
{
	...
	for(i=0;i<CHILDREN_MAX;++i)
	{
		...
		child[i]->field=(char(*)[WIDTH])malloc(sizeof(char)*HEIGHT*WIDTH);
		if(child[i]->lv<VISIBLE_BLOCKS-1)
			constructRecTree(child[i]);
	}
}

int checkScore(int lv, char f[HEIGHT][WIDTH], int r, int y, int x)
{
	return AddBlockToField(f,nextBlock[lv],r,y,x)+5*DeleteLine(f)+5*y+3*side_touched-3*downdown;
}

void printScorePerTime()
{
	move(23, WIDTH + 10);
	printw("TIME");
	DrawBox(24, WIDTH + 10, 1, 8);

	move(29, WIDTH + 10);
	printw("SCORE/TIME");
	DrawBox(30, WIDTH + 10, 1, 8);
...
int modified_recommend(RecNode* root, int lv)
{
	...
	for(r = 0; r < NUM_OF_ROTATE; r++)
	{
		...
		for(x = -3; x < WIDTH; x++)
		{
			...
			if(CheckToMove(root->field, nextBlock[lv], r, y, x) == 1)
			{
				for(j = 0; j < HEIGHT; j++)
					for(k = 0; k < WIDTH; k++)
						c[i]->field[j][k] = root->field[j][k];
				...
				if(c[i]->accumulatedscore - root->accumulatedscore <= 10)
					continue;
				if(lv == VISIBLE_BLOCKS - 1)
					max = c[i]->accumulatedscore;

				else
				{
					if(lv >= 2 && c[i]->accumulatedscore < (double)max * 0.2)
						continue;
					temp = modified_recommend(c[i], lv + 1);
					...
					else if(max == temp && lv == 0 && recRoot->recBlockY < y)
					{
						max = temp;
						if(lv == 0)
						{
